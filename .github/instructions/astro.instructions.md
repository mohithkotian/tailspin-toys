...
- Define and document a `Props` interface in frontmatter for every reusable
  component. Describe the component contract and each field; this is the
  component's public API.
- Type component imports and helper return values
- Comment intent and non-obvious decisions only; do not restate template
  markup. Keep comments current when changing the related component.
